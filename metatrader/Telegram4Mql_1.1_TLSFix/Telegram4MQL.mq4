...
void OnStart() {
  ...
  int i = 1;
  //---
  Print(TelegramSendTextAsync(apiKey, chatId, "test async text") + " async text");
  Print("async text sent");
  Sleep(2000);
  Print(TelegramSendScreenAsync(apiKey, chatId, "test async screen") + " async screen");
  Print("async screen sent");
  Sleep(2000);
  Print(TelegramSendPhotoAsync(apiKey, chatId, "F:\\Files\\GBPUSDH1.png", "test async photo") + " async photo");
  Print("async photo sent");
  Sleep(2000);
  Print(TelegramSendDocumentAsync(apiKey, chatId, "F:\\Files\\log.txt", "test async doc") + " async doc");
